...
def opt_parse(opt_path):
    ...
    return opt
def deg_pipe_1(source):
    """
    Degradation pipeline: Gaussian blur -> Random resize -> JPEG compression
    """
    # Gaussian blur
    k_size = random.randint(0, 10) * 2 + 1
    sigma = np.random.choice(np.arange(1, 15, step=0.1))
    source = cv2.GaussianBlur(source, (k_size, k_size), sigma)

    # Random resize
    ori_h, ori_w = source.shape[0], source.shape[1]
    scale_factor = random.uniform(0.5, 1.5)
    resize_mode = random.choice([cv2.INTER_NEAREST, cv2.INTER_LINEAR, cv2.INTER_CUBIC])
    resized_img = cv2.resize(source, None, fx=scale_factor, fy=scale_factor, interpolation=resize_mode)

    # JPEG compression and resize back
    quality_parameter = random.randint(50, 99)
    encode_param = [int(cv2.IMWRITE_JPEG_QUALITY), quality_parameter]
    if np.random.uniform() < 0.5:
        img = cv2.resize(resized_img, (ori_w, ori_h), interpolation=resize_mode)
        _, encimg = cv2.imencode('.jpg', img, encode_param)
        return cv2.imdecode(encimg, 1)
    else:
        _, encimg = cv2.imencode('.jpg', resized_img, encode_param)
        decimg = cv2.imdecode(encimg, 1)
        return cv2.resize(decimg, (ori_w, ori_h), interpolation=resize_mode)
class RealESRGAN_degradation(object):
    def __init__(self, opt_name='params_realesrgan.yml', device='cpu',vis_param=False):
    ...
    def degrade_process(self, img_gt, resize_bak=False):
        degra_info = [] if self.vis_param else None
        ...
        if self.vis_param:
            degra_info.append({'type': 'interpolate', 'params': [mode,scale]})
        # noise
        ...
        if np.random.uniform() < self.opt['gaussian_noise_prob']:
            ...
            if self.vis_param:
                degra_info.append({'type': 'gaussian_noise', 'params': self.opt['noise_range']})
        else:
            out = random_add_poisson_noise_pt(
                ...
                rounds=False)
            if self.vis_param:
                degra_info.append({'type': 'poisson_scale', 'params': self.opt['poisson_scale_range']})
        # JPEG compression
        jpeg_p = out.new_zeros(out.size(0)).uniform_(*self.opt['jpeg_range'])
        out = torch.clamp(out, 0, 1)
        out = self.jpeger(out, quality=jpeg_p)
        if self.vis_param:
            degra_info.append({'type': 'jpeger', 'params': jpeg_p})
        ...
        if np.random.uniform() < self.opt['second_blur_prob']:
            ...
            if self.vis_param:
                degra_info.append({'type': 'blur', 'params': kernel2.shape})
        # random resize
        ...
        if np.random.uniform() < self.opt['gaussian_noise_prob2']:
            ...
            if self.vis_param:
                degra_info.append({'type': 'gaussian_noise', 'params': self.opt['noise_range2']})
        else:
            out = random_add_poisson_noise_pt(
                out,
                ...
                rounds=False)
            if self.vis_param:
                degra_info.append({'type': 'poisson_scale', 'params': self.opt['poisson_scale_range2']})
        ...
        if np.random.uniform() < 0.5:
            ...
            out = self.jpeger(out, quality=jpeg_p)
            if self.vis_param:
                degra_info.append({'type': 'interpolate', 'params': [mode,scale_final]})
                degra_info.append({'type': 'blur', 'params': sinc_kernel.shape})
                degra_info.append({'type': 'jpeger', 'params': jpeg_p})
        else:
            # JPEG compression
            jpeg_p = out.new_zeros(out.size(0)).uniform_(*self.opt['jpeg_range2'])
        ...
        if np.random.uniform() < self.opt['gray_prob']:
            ...
            if self.vis_param:
                degra_info.append({'type': 'rgb2gray', 'params': True})
        ...
        if np.random.uniform() < self.opt['color_jitter_prob']:
            ...
            if self.vis_param:
                degra_info.append({'type': 'color_jitter', 'params': [brightness,contrast,saturation,hue]})

        if resize_bak:
            mode = random.choice(['area', 'bilinear', 'bicubic'])
            out = F.interpolate(out, size=(ori_h, ori_w), mode=mode)
            if self.vis_param:
                degra_info.append({'type': 'final_interpolate', 'params': [mode]})
        # clamp and round
        img_lq = torch.clamp((out * 255.0).round(), 0, 255) / 255.
        if self.vis_param:
            return img_gt, img_lq, degra_info
        else:
