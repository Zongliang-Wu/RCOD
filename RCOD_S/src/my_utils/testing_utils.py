...
def evaluate(in_path, ref_path, ntest=None, current_metric_dict=None, current_metric_paired_dict=None, fid_metric=None, device="cuda"):
    """
    Evaluates restoration metrics between generated images and reference GT images.
    """
    import pyiqa
    from utils import util_image

    if current_metric_dict is None:
        current_metric_dict = {
    ...
    if current_metric_paired_dict is None:
        current_metric_paired_dict = {
    ...
    lr_path_list = sorted([x for x in Path(in_path).glob("*.[jpJP][pnPN]*[gG]")])
    if ntest is not None:
        lr_path_list = lr_path_list[:ntest]

    for key in current_metric_dict.keys():
        current_metric_dict[key] = current_metric_dict[key].to(device)
    for key in current_metric_paired_dict.keys():
        current_metric_paired_dict[key] = current_metric_paired_dict[key].to(device)

    for i in range(len(lr_path_list)):
        ...
        for key, metric in current_metric_dict.items():
        ...
        if _ref_path is not None:
            ...
            for key, metric in current_metric_paired_dict.items():
                try:
                    result[key] = result.get(key, 0) + metric(im_in_tensor, im_ref_tensor).item()
                except Exception:
                    result[key] = result.get(key, 0)

    if fid_metric is not None and ref_path is not None:
        try:
            result['fid'] = fid_metric(str(in_path), str(ref_path))
        except Exception:
            result['fid'] = 0.0
    else:
        result['fid'] = 0.0

    num_imgs = max(len(lr_path_list), 1)
    avg_metrics_str = "; ".join([
        f"{k}: {result[k]:.2f}" if k == "fid" else f"{k}: {result[k]/num_imgs:.5f}"
        for k in result.keys()
    ])

    return avg_metrics_str, result, len(lr_path_list)
