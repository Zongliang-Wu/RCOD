...
def parse_args_paired_training(input_args=None):
   ...
   """
    ...
    parser.add_argument("--max_train_steps", type=int, default=50000,) #50000
    ...
    parser.add_argument("--n_step", default=249, type=int, help="Max gradient norm.")
    parser.add_argument("--n_div", default=3, type=int, help="Max gradient norm.")
    parser.add_argument("--model_name", default='s3diff', type=str, help="Max gradient norm.")
    parser.add_argument("--reduce_metrics", default=False, type=bool, help="Max gradient norm.")
    parser.add_argument("--model_name_score", default='No', type=str, help="Max gradient norm.")
    parser.add_argument("--mlp_path", type=str, default=None,)
    parser.add_argument("--img_neg", type=str, default="no", choices=["no", "black", "white","deg"],)
    parser.add_argument("--process_size", type=int, default=512, help='Size for processing')
    parser.add_argument('--inference_iterations', type=int, default=15, help='Number of inference iterations')
...
class TestDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if 'lr_path' in opt:
            if isinstance(opt['lr_path'], str):
                self.paths_lr.extend(sorted([str(x) for x in Path(opt['lr_path']).glob('*.'+opt['image_type'])]))
            else:
                self.paths_lr.extend(sorted([str(x) for x in Path(opt['lr_path'][0]).glob('*.'+opt['image_type'])]))
                if len(opt['lr_path']) > 1:
                    for i in range(len(opt['lr_path'])-1):
                        self.paths_lr.extend(sorted([str(x) for x in Path(opt['lr_path'][i+1]).glob('*.'+opt['image_type'])]))
    ...
    def __getitem__(self, index):
        ...
        while retry > 0:
            try:
                img_bytes = self.file_client.get(gt_path, 'gt')
                img_bytes_lr = self.file_client.get(lr_path, 'lr')
            except (IOError, OSError) as e:
                # logger = get_root_logger()
                # logger.warn(f'File client error: {e}, remaining retry times: {retry - 1}')
                # change another file to read
                ...
                retry -= 1
        img_gt = imfrombytes(img_bytes, float32=True)
        img_lr = imfrombytes(img_bytes_lr, float32=True)
        ...
        return_d = {'gt': img_gt, 'gt_path': gt_path,'lr': img_lr, 'lr_path': lr_path}
        return return_d
