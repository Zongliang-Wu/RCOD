...
class S3Diff(torch.nn.Module):
    ...
    @perfcount
    @torch.no_grad()
    def forward(self, c_t, deg_score):
 

        pos_caption_enc = self.get_prompt_encoder(c_t,batch_size=c_t.shape[0])
        ...
        if h * w <= tile_size * tile_size:
            model_pred = self.unet(lq_latent, self.timesteps, encoder_hidden_states=pos_caption_enc).sample
        else:
